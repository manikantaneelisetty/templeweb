...
<main class="bg-surface overflow-hidden pt-24 relative">
  ...
  <section class="w-full relative flex flex-col items-center bg-surface pt-4 pb-12 z-10">
  ...
  <!-- ADVANCED SCROLL INTERPOLATION & OBSERVER SCRIPT -->
  <script>
    (function() {
      // 1. Observer for smooth fades below the fold
      ...
      // 2. React-Safe FLIP Scroll Math for Traveling Image
      function initTraveler() {
        var traveler = document.getElementById('traveling-card');
        var startSlot = document.getElementById('card-start-slot');
        var endSlot = document.getElementById('card-end-slot');
        
        if (!traveler || !startSlot || !endSlot) return;
        if (traveler.dataset.initialized) return; // Prevent re-binding old closures if React renders twice
        
        traveler.dataset.initialized = 'true';
        
        // Hide the SSR fallback image and reveal the physical traveling card
        traveler.style.opacity = '1';
        startSlot.style.opacity = '0';

        // Bind global render function safely attached to window
        window.renderTraveler = function() {
          // Re-query nodes specifically to survive React unmounting/remounting
          var t = document.getElementById('traveling-card');
          var s = document.getElementById('card-start-slot');
          var e = document.getElementById('card-end-slot');
          if (!t || !s || !e) return;
          
          var startRect = s.getBoundingClientRect();
          var endRect = e.getBoundingClientRect();
          var scrollY = window.scrollY || document.documentElement.scrollTop;
          
          var startDocY = startRect.top + scrollY;
          var endDocY = endRect.top + scrollY;
          
          var travelStart = startDocY;
          var travelEnd = endDocY - 200; 
          
          var travelDistance = travelEnd - travelStart;
          if (travelDistance <= 0) travelDistance = 1;
          
          var progress = (scrollY - travelStart) / travelDistance;
          if (progress < 0) progress = 0;
          if (progress > 1) progress = 1;

          var eased = progress < 0.5 ? 2 * progress * progress : -1 + (4 - 2 * progress) * progress;
          
          var currentTop = startRect.top + (endRect.top - startRect.top) * eased;
          var currentLeft = startRect.left + (endRect.left - startRect.left) * eased;
          var currentWidth = startRect.width + (endRect.width - startRect.width) * eased;
          var currentHeight = startRect.height + (endRect.height - startRect.height) * eased;
          var currentRadius = 40 + (48 - 40) * eased;

          t.style.transform = 'translate3d(' + currentLeft + 'px, ' + currentTop + 'px, 0)';
          t.style.width = currentWidth + 'px';
          t.style.height = currentHeight + 'px';
          t.style.borderRadius = currentRadius + 'px';
        };

        // Attach event listeners once
        if (!window.travelerScrollBound) {
          window.travelerScrollBound = true;
          window.addEventListener('scroll', function() {
            if (window.renderTraveler) requestAnimationFrame(window.renderTraveler);
          }, { passive: true });
      ...
      // Continuous loop outlives Next.js unpredictable hydration timing
